...
export default function AlertBanner({ reason }: { reason: string | null }) {
  return (
    <div className="flex flex-col items-start justify-between gap-3 rounded-xl border border-alert-500/30 bg-gradient-to-r from-alert-500/10 to-transparent px-4 py-3 sm:flex-row sm:items-center">
      ...
      <a
        href={CONTACT_HREF}
        className="shrink-0 rounded-lg border border-alert-500/40 px-3 py-1.5 text-xs font-semibold text-alert-300 transition-colors hover:bg-alert-500/10 whitespace-nowrap"
      >
        Contact us →
      </a>
    </div>
